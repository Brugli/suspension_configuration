...
func _physics_process(delta: float) -> void:
	...
	if chassis.rear_suspension_rest_distance > chassis.front_suspension_rest_distance:
		front_angle = -asin(abs(abs(chassis.rear_suspension_rest_distance)-abs(chassis.front_suspension_rest_distance)) / lenght_of_vehicle)
		rear_angle = -asin(abs(abs(chassis.front_suspension_rest_distance)-abs(chassis.rear_suspension_rest_distance)) / lenght_of_vehicle)
	else:
		front_angle = asin(abs(abs(chassis.front_suspension_rest_distance)-abs(chassis.rear_suspension_rest_distance)) / lenght_of_vehicle)
		rear_angle = asin(abs(abs(chassis.rear_suspension_rest_distance)-abs(chassis.front_suspension_rest_distance)) / lenght_of_vehicle)
...
func suspension(delta, collision_point):
	...
	spring_length = clamp(distance - wheel_radius, 0, suspension_rest_distance)
	spring_force = chassis.spring_strength * (suspension_rest_distance - spring_length)
